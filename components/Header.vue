...
<template>
  <header class="header-glass shadow-sm border-b border-neutral-200/50 dark:border-neutral-700/50 h-16 fixed w-full z-30">
    <div class="w-full h-full flex items-center px-2 md:px-4 gap-1 md:gap-2">
      ...
      <h1
        @click="handleReload"
        class="text-base md:text-xl font-bold ml-1 md:ml-2 hover:cursor-pointer bg-clip-text text-transparent bg-gradient-to-r from-slate-700 to-slate-500 dark:from-slate-300 dark:to-slate-400 transition-all duration-200 hidden sm:block flex-shrink-0"
      >
        Deno Chat
      </h1>
      ...
      <UButton 
        size="sm" 
        class="ml-1 md:ml-4 px-2 md:px-4 py-1.5 md:py-2 text-xs md:text-sm bg-slate-700 hover:bg-slate-800 text-white rounded-lg md:rounded-xl font-medium shadow-lg shadow-slate-500/20 transition-all duration-300 btn-scale dark:bg-slate-300 dark:hover:bg-slate-200 dark:text-slate-900 flex-shrink-0" 
        @click="openModelSelect = true"
      >
        <span class="max-w-[80px] md:max-w-none truncate">{{ selectedModel.name }}</span>
        <template #trailing>
          <UIcon name="i-heroicons-chevron-down-solid" class="w-3 h-3 ml-0.5 md:ml-1 flex-shrink-0"/>
        </template>
      </UButton>

      <div class="ml-auto flex items-center gap-0.5 md:gap-1 flex-shrink-0">
        <a href="https://github.com/zero456/deno-ai-chat" target="_blank" aria-label="GitHub"
           class="p-2 rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-700 transition-colors duration-200 hidden md:flex">
